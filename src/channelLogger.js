    ...
    createLogEmbed(userId, username, channelId, channelName, action, additionalInfo) {
        ...
        switch (action) {
            ...
            case 'MOVE':
                ...
                // Add XP information if available from previous session
                if (additionalInfo.xpEarned !== undefined && additionalInfo.xpEarned > 0) {
                    let xpText = `+${additionalInfo.xpEarned} XP (from previous channel)`;
                    
                    if (additionalInfo.xpCapHit) {
                        const capEmoji = additionalInfo.capType === 'daily' ? '📅' : 
                                       additionalInfo.capType === 'weekly' ? '📆' : 
                                       additionalInfo.capType === 'monthly' ? '🗓️' : '🚫';
                        xpText += ` ${capEmoji} (${additionalInfo.capType} cap)`;
                    }
                    
                    embed.addFields({ name: '⚡ XP from Previous Session', value: xpText, inline: true });
                }
                break;
    ...
    async createLogChannel(guild) {
        try {
            ...
            const newChannel = await guild.channels.create({
                ...
                type: 0, // Text channel
                topic: 'Automatic voice activity logging with XP tracking - Join/Leave/Move events',
